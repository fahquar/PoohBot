...
class Wolfram(callbacks.Privmsg):
    ...
    def wolfram(self, irc, msg, args, question):
        ...
        if tree.attrib['success'] == "false":
            for results in tree.findall('.//error'):
                for err in results.findall('.//msg'):
                    ...
                    return
            irc.reply("Sorry, I don't know how to interpret your input.", prefixNick=True)
            return
            ...
            for plaintext in pod.findall('.//plaintext'):
                if plaintext.text:
                    ...
                    if outputcount < maxoutput:
                        ...
                        outputcount += 1
        if not found:
            irc.reply("Sorry, I wasn't able to find any results for that query.", prefixNick=True)

    wolfram = wrap(wolfram, ['text'])
